...
def read_seeds(root_dir, archive_name, dataset_name, itr):
    file_name = root_dir + '/seeds/' + archive_name + '/' + dataset_name + '/init_clusters_not_per_class/'
...
def noise(code, type='uniform'):
    ...
    if type == 'laplace':
        noise_code = noise_code + np.random.laplace(loc=0.0, scale=1.0, size=noise_code.shape)
    elif type == 'drop':
        for i in range(len(noise_code)):
            idx = np.around(np.random.uniform(0., code.shape[1] - 1, size=int(code.shape[1] * 0.2))).astype(np.int)
            noise_code[i, idx] = 0
    return noise_code
...
def cdist_dtw(dataset1, dataset2=None, global_constraint=None,
    ...
    if dataset2 is None:
        ...
        with Pool(processes=n_jobs) as pool:
            matrix = pool.map(partial(dtw_row, dataset=dataset2,
                                      global_constraint=global_constraint,
                                      sakoe_chiba_radius=sakoe_chiba_radius,
                                      itakura_max_slope=itakura_max_slope), dataset1)
        return np.array(matrix).reshape((len(dataset1), -1))


def dtw_row(ts, dataset, global_constraint=None, sakoe_chiba_radius=None, itakura_max_slope=None):
    row = [dtw(ts, dataset[j],
               global_constraint=global_constraint,
               sakoe_chiba_radius=sakoe_chiba_radius,
               itakura_max_slope=itakura_max_slope)
...
def load_graph(data_size, path):
    ...
    adj = sp.coo_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])),
                        shape=(data_size, data_size), dtype=dtype)
